function [ centerLeft,centerRight, Aspect_ratio, texture_rectangle]=CalibrateMirrors3(windowPtr,EXPCONSTANTS)
...
% if stereoMode<3
%     Aspect_ratio=RectWidth(windowRect)/(2*RectHeight(windowRect));
% else
%    Aspect_ratio=RectWidth(windowRect)/(RectHeight(windowRect));
% end
Aspect_ratio=EXPCONSTANTS.AR_correction;
...
if stereoMode<4
texture_rectangle=[text_r(1,1)-0.5*Size text_r(1,2)-0.5*Size/Aspect_ratio text_r(1,1)+0.5*Size text_r(1,2)+0.5*Size/Aspect_ratio];
else
...
%Run until 'q' or 'space' is pressed  
    ...
    Screen('DrawTexture',windowPtr,text_index,[],TR,[],[],[],EXPCONSTANTS.CALIBRATE_IMAGE_LEFT_COLOUR); 
    %Draw also a frame on the texture
    Screen('FrameRect', windowPtr ,EXPCONSTANTS.CALIBRATE_FRAME_COLOUR, TR, EXPCONSTANTS.CALIBRATE_FRAME_WIDTH);
    Screen('DrawLines',windowPtr,EXPCONSTANTS.FIX_POSITION,EXPCONSTANTS.FIXCROSSWIDE,EXPCONSTANTS.FIXATION_COLOUR,[0 0]);
    % Select right-eye image buffer for drawing:
    Screen('SelectStereoDrawBuffer', windowPtr, 1);
    
    % Draw right stim:
    TR=texture_rectangle;
    Screen('DrawTexture',windowPtr,text_index,[],TR,[],[],[],EXPCONSTANTS.CALIBRATE_IMAGE_RIGHT_COLOUR); 
    Screen('FrameRect', windowPtr ,EXPCONSTANTS.CALIBRATE_FRAME_COLOUR, TR, EXPCONSTANTS.CALIBRATE_FRAME_WIDTH);
    Screen('DrawLines',windowPtr,EXPCONSTANTS.FIX_POSITION,EXPCONSTANTS.FIXCROSSWIDE,[255 0 0],[0 0]);
