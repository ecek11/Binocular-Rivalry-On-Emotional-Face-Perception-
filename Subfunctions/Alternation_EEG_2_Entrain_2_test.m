function [T0, T_alternation,key_press_stored,Actual_frequency,Entrain_T]=Alternation_EEG_2_Entrain_2_test(EXPCONSTANTS,GaborBuffer,RadialBuffer,mode,ioObj,address)
...
for nc=1:N_textures
   ...
   % if nc==1
    ...
    Text(nc,1).gaborTextureHorizontal=Screen('MakeTexture',wPtr,gaborPatchHor);
    Text(nc,1).gaborTextureVertical=Screen('MakeTexture',wPtr,gaborPatchVer);
end
...
Screen('FillRect', screenPointer, EXPCONSTANTS.BACKGROUNDCOLOR);
Screen('DrawTextures', screenPointer,  Text(N_textures).gaborTextureHorizontal ,[] , texture_rectangleB0, [], [],alphaBack); %0.
...
% Screen('FillRect', screenPointer, EXPCONSTANTS.BACKGROUNDCOLOR);
Screen('DrawTextures', screenPointer,  Text(N_textures).gaborTextureVertical ,[] , texture_rectangleB1, [], [],alphaBack); %0.
...
while not(exit_loop)
    ...
    while (finTime<EXPCONSTANTS.ALTERNATION_MAXTIME)
        ...
        for j=1:N_textures
            ...
            Screen('FillRect', screenPointer, EXPCONSTANTS.BACKGROUNDCOLOR);
            Screen('DrawTextures', screenPointer,  Text(j).gaborTextureHorizontal ,[] , texture_rectangleB0, [], [],alphaBack); %0.
            %Screen('DrawTextures', win, gabortex, srcRect, dstRects, [], [], ALPHA);
            %win: Monitor pointer
            %gabortex: Textures
            %srcRect: part of the texture to be drawn []: draw all the texture
            ...
            % Screen('SelectStereoDrawBuffer', screenPointer, RadialBuffer);%Buffer 1
            % Screen('FillRect', screenPointer, EXPCONSTANTS.BACKGROUNDCOLOR);
            Screen('DrawTextures', screenPointer,  Text(j).gaborTextureVertical ,[] , texture_rectangleB1, [], [],alphaBack); %0.
            %Screen('FrameOval', screenPointer, EXPCONSTANTS.BACKGROUNDCOLOR,TRbackR, Width_back);
            Screen('FillOval', screenPointer, [50 50 50],TR2R, EXPCONSTANTS.DOT_FRAME_WIDTH);
            Screen('DrawLines',screenPointer,EXPCONSTANTS.FIX_POSITION+[xmax/2 xmax/2 xmax/2 xmax/2;0 0 0 0],EXPCONSTANTS.FIXCROSSWIDE,EXPCONSTANTS.FIXATION_COLOUR,EXPCONSTANTS.FIXATION_CENTRE_B1');
            Screen('FrameOval', screenPointer, EXPCONSTANTS.DOT_FRAME_COLOUR,TRR, EXPCONSTANTS.DOT_FRAME_WIDTH);
...
for nc=1:length(Text)
    Screen('Close', Text(nc,1).gaborTextureHorizontal);
    Screen('Close',Text(nc,1).gaborTextureVertical);
end
